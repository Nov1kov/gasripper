...
fn inverse_guards(
    ...
    for p in 0..n - 1 {
        ...
        let ends_revert = block.last().is_some_and(|&l| item_name(&code[l]) == "REVERT");
...
fn descriptor(item: &Value, revert: &HashSet<String>) -> (&'static str, String, Option<String>) {
    ...
    if name == "tag" {
        return ("label", format!("_sym_tag_{}", item_value(item).unwrap_or_default()), None);
    }
    if name == "JUMPDEST" {
        return ("label", "JUMPDEST".to_string(), None);
    }
    if name == "PUSH" {
        return ("push", "PUSH".to_string(), Some(format!("0x{}", item_value(item).unwrap_or_default())));
    }
...
mod tests {
    ...
    fn drop_of_revert_block_wins_over_a_replacement_inside_it() {
        ...
        let stranded = out
            .iter()
            .any(|i| item_value(i).as_deref() == Some("abcd"));
        assert!(
            !stranded,
